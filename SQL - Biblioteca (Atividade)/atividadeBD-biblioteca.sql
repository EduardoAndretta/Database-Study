...
SELECT nome_livro Livro, paginas Paginas, nome_categoria Categoria FROM tbl_livros
...
SELECT nome_categoria FROM tbl_categoria 
	LEFT JOIN tbl_livros ON FK_Categoria = codigo_categoria
    WHERE FK_Categoria IS NULL OR codigo_categoria IS NULL;
    
#4 - Exibir o nome dos livros cadastrados, juntamente com suas categorias e nome das editoras que o produziram
SELECT nome_livro Livro, nome_categoria Categoria, nome_editora Editora FROM tbl_livros
	JOIN tbl_categoria ON FK_Categoria = codigo_categoria
...
SELECT nome_autor Autor, count(*) Livros FROM tbl_livros
	JOIN autor_livro ON codigo_livro = FK_Livro
    JOIN tbl_autor ON FK_Autor = codigo_autor
    JOIN tbl_categoria ON FK_Categoria = codigo_categoria
    WHERE nome_categoria IN("Romance")
    GROUP BY nome_autor ORDER BY Livros desc;
    
SELECT nome_autor Autor, nome_categoria Categoria FROM tbl_livros
	JOIN autor_livro ON FK_Livro = codigo_livro 
    JOIN tbl_autor ON FK_Autor = codigo_autor
    JOIN tbl_categoria ON FK_Categoria = codigo_categoria;

#6- Exibir os nomes dos autores e o nome dos seus livros.
SELECT nome_autor Autor, nome_livro Livro FROM tbl_livros
	JOIN autor_livro ON FK_Livro = codigo_livro
    JOIN tbl_autor ON FK_Autor = codigo_autor;
...
#8 - EXIBIR APENAS O NOME, A SUA CATEGORIA E VALOR_UNITARIO PARA TODOS OS LIVROS QUE O VALOR UNITARIO FOR SUPERIOR A MEDIA DOS
#PREÇOS. ORDENAR A BUSCA DO MENOR VALOR PARA O MAIOR. # DICA: UTILIZE SUBQUERY E ORDER BY.
SELECT nome_livro Livro, nome_categoria Categoria, valor_unitario ValorUnidade FROM tbl_livros
	JOIN tbl_categoria ON FK_Categoria = codigo_categoria
...
SELECT a.nome_livro Livro, i.nome_categoria Categoria, e.nome_autor Autor, g.nome_editora Editora, 
g.CNPJ CNPJEditora, h.nome_cliente Cliente, b.retirada Retirada, b.devolucao Devolução, 
c.nome_bibliotecario Bibliotecario, c.telefone Telefone FROM tbl_livros a
	JOIN tbl_categoria i ON a.FK_Categoria = i.codigo_categoria 
    JOIN tbl_editora g ON a.FK_Editora = g.codigo_editora
    JOIN autor_livro f ON f.FK_Livro = a.codigo_livro
    JOIN tbl_autor e ON f.FK_Autor = e.codigo_autor
    JOIN livro_emprestimo d ON d.FK_Livro = a.codigo_livro
    JOIN tbl_emprestimo b ON d.FK_Emprestimo = b.codigo_emprestimo
    JOIN tbl_cliente h ON b.FK_Cliente = h.codigo_cliente
    JOIN tbl_bibliotecario c ON b.FK_Bibliotecario = c.codigo_bibliotecario
    WHERE year(h.nasc_cliente) > 2000 AND year(c.nasc_bibliotecario) < 2000
    ORDER BY nome_livro;